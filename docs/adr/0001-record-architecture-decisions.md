...
- **ADR-0003 — Single Tauri desktop shell** — *Proposed.* Ship one artifact (`PulseTrader.app`): a Rust backend plus a TypeScript/React UI in a WKWebView, glued by a tauri-specta-generated command bus, with no sidecar processes. Rationale: a single code-signed, notarized, auto-updating native artifact with no Python runtime to bundle or secure; the CLI proof-of-concept and the GUI share one Rust core. *(2026-08-23: this entry was compound. The **zero-sidecar** half is written up and Accepted in ADR-0015 and is no longer part of this placeholder. The **Tauri desktop shell** is what remains Proposed — unbuilt at baseline `49f229a`, and deliberately NOT decided by ADR-0019, which scopes itself to the exercised stack. It needs its own ADR when a slice builds the shell.)*
