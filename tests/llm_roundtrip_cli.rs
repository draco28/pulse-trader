...
/// (`glm-5.3`, README C5), CNY-native — so the computed `cost` is populated +
/// non-zero. These are TEST values, NOT production moat data (the real
/// coding-plan rate is a nominal estimate, config-tunable).
