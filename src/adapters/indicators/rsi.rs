...
impl Indicator for Rsi {
    fn next(&mut self, candle: &Candle) -> Option<Decimal> {
        ...
        // Feed every candle so the recursive ta-rs EMA state warms, even while we
        // are still suppressing output during warmup. A non-representable price
        // maps to `None` defensively (never a panic).
